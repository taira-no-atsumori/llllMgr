        ...
        <v-card variant="flat">
          ...
          <v-card-title class="text-left">
            ...
            <div v-if="event.state === 'prev'">
              {{ event.text }}まで<span class="d-inline-block">あと<template v-if="event.count.day > 0"><b class="text-red">{{ event.count.day }}</b>日</template><template v-else><b class="text-red">{{ event.count.time }}</b>時間</template></span>
            </div>
            <div v-else>
              {{ event.text }}<span class="d-inline-block"><b class="text-red">開催<span v-if="event.type === 'live'">日</span><span v-else>中</span></b></span>
            </div>
          </v-card-title>
        </v-card>
      </v-carousel-item>
...
<v-row>
  ...
  <v-col>
    <v-btn block prepend-icon="mdi-calculator" @click="pageMove('simulation')" :disabled="true">Simulation</v-btn>
  </v-col>
  <v-col v-if="false">
...
<v-row>
  ...
  <v-col cols="12">
    <s><b>SIMULATION(獲得グランプリPt.計算ツール)</b><br>
    獲得グランプリPt.を計算できます。<br>
    なお、簡単な編成シミュレーションと編成情報の保存機能を搭載し、リニューアルする予定です。</s><br>
    →リニューアル準備中
  </v-col>
  <v-col cols="12">
...
export default {
  ...
  data() {
    return {
      eventList: {
        'liveGP': {
          title: 'ライブグランプリ「104期1stTerm第4回個人戦」',
          text: '',
          type: 'liveGP',
        ...
        /*'liveGP': {
          title: 'ライブグランプリ「104期 1stTerm 第3回サークル対抗戦」',
          text: '',
          type: 'liveGP',
          firstDay: [2024, 6, 22, 12, 0],
          lastDay: [2024, 6, 28, 3, 59],
          url: 'https://www.lovelive-anime.jp/hasunosora/appnews/detail/?p=2024-04-19-30-la8hkbjnyy',
          img: '104期 1stTerm 第3回サークル対抗戦_logo'
        },*/
        'anisama': {
        ...
        '3rdLiveTour_みらくらぱーく！': {
          ...
          text: 'with みらくらぱーく！',
          type: 'live',
          firstDay: [2024, 11, 16, 0, 0],
          lastDay: [2024, 11, 16, 23, 59],
          url: 'https://www.lovelive-anime.jp/hasunosora/live-event/live_detail.php?p=TTUM',
          img: '3rdLiveTour_logo_みらくらぱーく！'
        },
  ...
  methods: {
    ...
    countDown(eventName) {
      ...
      const lastDay = new Date(this.eventList[eventName].lastDay[0], (this.eventList[eventName].lastDay[1] - 1), this.eventList[eventName].lastDay[2], this.eventList[eventName].lastDay[3], this.eventList[eventName].lastDay[4], 59);
